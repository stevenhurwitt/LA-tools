...
<form method="post" action="/services/accounts/">
  ...
  <br><br>
  <b>Name and email:</b> <input size="50" type="text" name="email"/> <br/>
...
""".format(**env))
            if send_email and env["email"]:
                message = """
You signed up for a Jupyterhub associated with Engie, NA.

Username: {username}
Password: {password}

If you have any questions, please check with:
{stevenhurwitt}

Thank you!
"""
                env["message"] = message.format(**env)
                system('echo -e "{message}" | mail -s "Jupyter Access - Engie, NA" {email}'.format(**env))
