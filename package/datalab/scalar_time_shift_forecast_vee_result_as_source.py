...
class ScalarTimeShiftForecastVeeResultAsSource(AbstractRule):
    """
    ...
    def apply(self, forecast_till=None, months_forward=None, reset_month_day=None, from_start_of_year=None, **kwargs):
        """
        1) calculate the factors VEE_RESULT/aggregated_profile_usage
        ...
        data_frame = self.dataframe[[self.source_column, self.source_heartbeat_column]].copy()
        scalar_df = self.calculate_start_column(data_frame)
        start = min(end - pd.DateOffset(months=13), scalar_df.loc[scalar_df.index[0], 'START'])
        ...
        forecast_start = end + one_hour
        if from_start_of_year:
            forecast_start = timezone.localize(dt.datetime(end.year, 1, 1)).astimezone(pytz.utc) + one_hour
        ...
        factors_df = self.prepare_factors_df(profile_df, scalar_df, self.source_column)
        # Start time-shifting
        factors_df = self.time_shift_factors_df(factors_df, forecast_start, forecast_end)

        rule_name = self.get_function_name()
        profile_df.loc[:, rule_name] = profile_df.loc[:, 'Usage'] * factors_df.loc[:, 'factors']
        forecast_df = profile_df.loc[forecast_start: forecast_end, [rule_name]]
        ...
        if len(forecast_df[forecast_df.index.duplicated()]) > 0:
            logger.info('removed %s duplicte values', len(forecast_df.index.duplicated()))
            forecast_df = forecast_df[~forecast_df.index.duplicated()]
        logger.info("final forecast has %s data points", len(forecast_df))
    ...
    def prepare_factors_df(profile_df, scalar_df, data_column):
        ...
        Args:
            ...
            scalar_df: a dataframe of scalar
        ...
        logger.info("Weighted average factor: {} used to fill the missing values".format(correction_factor))
    ...
    def calculate_start_column(self, df):
        ...
        import pandas as pd
        import numpy as np
        df = df[np.isfinite(df[self.source_heartbeat_column])]
        df.loc[:, 'START'] = [x - pd.Timedelta(seconds=y) for (x, y) in zip(df.index, df[self.source_heartbeat_column])]
